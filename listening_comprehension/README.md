...
Static CLA-style listening exercises for the GitHub Pages site.
...
python tools/generate_listening_audio.py \
  listening_comprehension/data/cla-b2-001.json \
  --gap-ms 140
```
...
```text
dist/audio/cla-b2-001.mp3
dist/audio/cla-b2-001.mp3.sha256
```
...
Use stable IDs such as `cla-b2-001` through `cla-b2-050`. Keep source JSON in Git, while generated MP3 and SHA-256 files live as release assets. For a future storage migration (for example to object storage), only the `audio.url` and `audio.sha256Url` fields need to change.
